...
parser.add_argument("--task", default="pre-trained-3", help="path to folder containing the model")
...
parser.add_argument("--is_hyper", default=0, type=int, help="use hypercolumn or not")
...
if is_training:
    ...
    os.environ['CUDA_VISIBLE_DEVICES']='3'
EPS = 1e-12
channel = 32 # number of feature channels to build the model, set to 64
...
def build(input):
    ...
    print('hyper:',hyper)
    if hyper:
        print("[i] Hypercolumn ON, building hypercolumn featuress ... ")
        ...
        print('[i] Hypercolumn features OFF ... ')
        #vgg19_features=build_vgg19(input[:,:,:,0:3]*255.0)
        #for layer_id in range(1,6):
            #vgg19_f = vgg19_features['conv%d_2'%layer_id]
            #input = tf.concat([tf.image.resize_bilinear(tf.zeros_like(vgg19_f),(tf.shape(input)[1],tf.shape(input)[2]))/255.0,input], axis=3)
            #input = tf.concat([tf.image.resize_bilinear((tf.shape(input)[1],tf.shape(input)[2]))/255.0,input], axis=2)
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    print('three loss')
    loss_percep_t=compute_l1_loss(transmission_layer, target)
    loss_percep_r=tf.where(issyn, compute_l1_loss(reflection_layer, reflection), 0.)
    ...
    #loss=loss_l1_r+loss_grad
    #for var in tf.trainable_variables():
    #    print("Listing1 trainable variables ... ")
    #    print(var)
...
# test doesn't need to load discriminator
    ...
    #print('loadedd '+ckpt.model_checkpoint_path)
    #saver_restore.restore(sess,ckpt.model_checkpoint_path)
...
if is_training:
    ...
    for epoch in range(1,maxepoch):
        ...
        for id in np.random.permutation(num_train):
            st=time.time()
            print('id:%d cnt:%d',id,cnt)
            if input_images[id] is None:
                magic=np.random.random()
                if magic < 0.7: # choose from synthetic dataset
                    is_syn=True
                    _synid=id%len(syn_image1_list)
                    syn_image1=cv2.imread(syn_image1_list[_synid],-1)
                    neww=np.random.randint(256, 480) #[256,480]
                    newh=int(round((neww/syn_image1.shape[1])*syn_image1.shape[0]))
                    output_image_t=cv2.resize(np.float32(syn_image1),(neww,newh),cv2.INTER_CUBIC)/255.0
                    output_image_r=cv2.resize(np.float32(cv2.imread(syn_image2_list[_synid],-1)),(neww,newh),cv2.INTER_CUBIC)/255.0
                    file=os.path.splitext(os.path.basename(syn_image1_list[_synid]))[0]
                    ...
                    neww=np.random.randint(256, 480) #[256,480]
                    newh=int(round((neww/inputimg.shape[1])*inputimg.shape[0]))
...
# To test the model on images with reflection
    ...
    test_path = ["./test_images/1/"] #["./test_images/real/"]
    subtask="1" # if you want to save different testset separately
